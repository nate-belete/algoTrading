...
class roiBands(bt.Strategy):
    ...
    def next(self):
        ...
        if len(self.close_) > 15 :
            self.close_p15_max.append(np.max(self.close_[-15:]))
            self.close_p15_min.append(np.min(self.close_[-15:]))
            ...
            self.roi_from_low_90th_pct.append(np.quantile(self.roi_from_low,0.80))

            # print(self.date_[-1])
            # print(self.close_[-1])
            # print(self.close_p15_max[-1])
            # print(self.roi_from_low_90th_pct[-1])

            if (self.roi_from_high[-1] <= self.roi_from_high_10th_pct[-1]) & (self.roi_from_low[-1] < self.roi_from_low_90th_pct[-1]):
        ...
        data = {'Date': self.date_[15:],
                'Close': self.close_[15:],
